...
class EuronewsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EuronewsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EuronewsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EuronewsTestRunner.getenv("EURONEWS_TEST_LIVE")
        override = EuronewsTestRunner.getenv("EURONEWS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EuronewsTestRunner.getenv(key)
        ...
        explain = EuronewsTestRunner.getenv("EURONEWS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["EURONEWS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EuronewsTestRunner._test_control is not None:
            return EuronewsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EuronewsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EuronewsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EuronewsTestRunner.load_test_control()
...
def load_env_local():
    EuronewsTestRunner.load_env_local()


def env_override(m):
    return EuronewsTestRunner.env_override(m)


def entity_data(v):
    return EuronewsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EuronewsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EuronewsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EuronewsTestRunner.load_test_control()


def live_client_options():
    return EuronewsTestRunner.live_client_options()


def live_delay_ms():
    return EuronewsTestRunner.live_delay_ms()
