...
[MCP](https://modelcontextprotocol.io) server exposing the Euronews SDK as
two agent tools — `euronews_list` and `euronews_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/euronews-mcp)
make build

# 2. Provide credentials via the environment
export EURONEWS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user euronews \
  -- /absolute/path/to/euronews-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./euronews-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// euronews_list: first page of records
{ "entity": "new" }
{ "entity": "new", "query": { } }

// euronews_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/euronews-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user euronews \
     -- "$PWD"/dist/*/euronews-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `euronews_list` and `euronews_load` tools now appear
   in new sessions. Ask the agent to *"list new using euronews"*
   and it calls `euronews_list` with `{"entity":"new"}`.
...
```sh
export EURONEWS_APIKEY=sk_live_xxx            # API key
export EURONEWS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `euronews_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `euronews_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EURONEWS_APIKEY` | API key sent with every request. |
| `EURONEWS_BASE` | Optional override of the API base URL. |
...
```sh
./euronews-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"euronews_load","arguments":{"entity":"new","query":{"id":1}}}}'
```
