...
function isRunnable(code) {
    return (/new\s+EuronewsSDK\b/.test(code) ||
        /\bEuronewsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+EuronewsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/EuronewsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
