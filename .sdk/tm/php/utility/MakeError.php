...
class EuronewsMakeError
{
    public static function call(?EuronewsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new EuronewsContext([], null);
        }
        $op = $ctx->op ?? new EuronewsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new EuronewsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof EuronewsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new EuronewsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof EuronewsError) {
