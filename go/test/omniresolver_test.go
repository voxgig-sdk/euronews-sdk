...
import (
	...
	omni "github.com/voxgig-sdk/euronews-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EuronewsError); is {
				out := map[string]any{
					"name":    "EuronewsError",
...
func omniCtx(arg any, client *sdk.EuronewsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EuronewsSDK); is {
			client = live
