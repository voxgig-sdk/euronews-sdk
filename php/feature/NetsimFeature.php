...
class EuronewsNetsimFeature extends EuronewsBaseFeature
{
    ...
    private function _simulate(EuronewsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(EuronewsContext $ctx, array $applied): void
    {
        $client = $this->client;
