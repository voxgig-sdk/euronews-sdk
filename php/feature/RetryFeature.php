...
class EuronewsRetryFeature extends EuronewsBaseFeature
{
    ...
    public function init(EuronewsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EuronewsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EuronewsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EuronewsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
