...
class NewEntity
{
    ...
    {
        if ($args) {
            $this->_data = EuronewsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = EuronewsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws EuronewsError on failure (item-5 convention).
     */
