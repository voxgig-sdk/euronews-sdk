...
{
    private static function utility(): EuronewsUtility
    {
        return new EuronewsUtility();
    }
    ...
    private static function ctx(array $over = []): EuronewsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EuronewsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EuronewsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EuronewsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EuronewsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EuronewsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EuronewsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EuronewsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EuronewsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EuronewsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EuronewsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EuronewsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new EuronewsResponse([]),
            'result' => new EuronewsResult([]),
        ]);
        [, $err] = EuronewsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EuronewsSpec([]),
            'response' => null,
            'result' => new EuronewsResult([]),
        ]);
        [, $err] = EuronewsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EuronewsSpec(['step' => 's']),
            'response' => new EuronewsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EuronewsResult([]),
        ]);
        [, $err] = EuronewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EuronewsSpec(['step' => 's']),
            'response' => new EuronewsResponse(self::resp(200, ['v' => 1])),
            'result' => new EuronewsResult([]),
        ]);
        [, $err] = EuronewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EuronewsSpec(['step' => 's']),
            'response' => new EuronewsResponse(self::resp(200, ['v' => 2])),
            'result' => new EuronewsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EuronewsMakeResponse::call($ctx);
    ...
    {
        $preset = new EuronewsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EuronewsSpec([]),
            'response' => new EuronewsResponse([]),
            'result' => new EuronewsResult([]),
        ...
        [$got, $err] = EuronewsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EuronewsResult([])]);
        [, $err] = EuronewsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EuronewsSpec([]), 'result' => null]);
        [, $err] = EuronewsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EuronewsSpec(['step' => 's']),
            'result' => new EuronewsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EuronewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EuronewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EuronewsSpec(['step' => 's']),
            'result' => new EuronewsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EuronewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EuronewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EuronewsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EuronewsSpec([]),
            'result' => new EuronewsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EuronewsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EuronewsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EuronewsError('boom', 'boom');
        $utility->fetcher = function (EuronewsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EuronewsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EuronewsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EuronewsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EuronewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EuronewsResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EuronewsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EuronewsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EuronewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EuronewsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EuronewsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EuronewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EuronewsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EuronewsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EuronewsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EuronewsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EuronewsResult(['ok' => false])]);
        $this->expectException(EuronewsError::class);
        EuronewsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EuronewsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EuronewsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EuronewsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EuronewsBaseFeature();
        $b = new EuronewsBaseFeature();
        EuronewsFeatureAdd::call($ctx, $a);
        EuronewsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EuronewsBaseFeature
    {
        $f = new EuronewsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EuronewsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EuronewsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EuronewsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EuronewsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EuronewsResponse(['headers' => null]),
            'result' => new EuronewsResult([]),
        ]);
        EuronewsResultHeaders::call($ctx);
