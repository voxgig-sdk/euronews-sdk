...
class FtCtrl extends EuronewsControl
{
...
class FtClient
{
    ...
    private EuronewsUtility $utility;

    public function __construct(EuronewsUtility $utility, array $options)
    {
    ...
    public function get_utility(): EuronewsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EuronewsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EuronewsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EuronewsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EuronewsConfig::make_feature($name);
    ...
    private static function buildUrl(EuronewsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EuronewsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EuronewsResponse($fetched);
        }
    ...
    private function fail(EuronewsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EuronewsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
