...
// Euronews SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = EuronewsOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(EuronewsOmni::structfix($now, Struct::undef()))
                        !== json_encode(EuronewsOmni::structfix($val, Struct::undef()))) {
