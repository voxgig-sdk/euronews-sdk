// Command Euronews-cli is a CLI + REPL for the Euronews SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EURONEWS_APIKEY (api key), EURONEWS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EURONEWS_APIKEY carries the API key and
	// EURONEWS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EURONEWS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EURONEWS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEuronewsSDK(opts)
