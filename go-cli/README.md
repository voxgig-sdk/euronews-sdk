...
boru-driven command-line client **and** interactive REPL for the Euronews
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/euronews-cli)
make build
...
./euronews-cli list new

# 5. Override the API base URL for a single call
EURONEWS_BASE=https://api.example.com ./euronews-cli list new
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/euronews-cli
   ```
...
Run with no arguments to open a REPL (prompt `euronews>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/euronews-cli`. |
...
(`./euronews-cli <expr>`) and interactively (the REPL), and expressions compose the
