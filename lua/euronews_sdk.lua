...
local EuronewsSDK = {}
EuronewsSDK.__index = EuronewsSDK
...
EuronewsSDK._make_feature = _make_feature


function EuronewsSDK.new(options)
  local self = setmetatable({}, EuronewsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function EuronewsSDK:options_map()
...
function EuronewsSDK:get_utility()
  return Utility.copy(self._utility)
end


function EuronewsSDK:get_root_ctx()
  return self._rootctx
end


function EuronewsSDK:prepare(fetchargs)
...
function EuronewsSDK:direct(fetchargs)
...
-- by passing it.
function EuronewsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function EuronewsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "EuronewsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function EuronewsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = EuronewsSDK.new(sdkopts)
  sdk.mode = "test"
