...
module EuronewsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = EuronewsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EuronewsResponse.new(fetched)
    else
