...
require_relative 'Euronews_types'
...
class EuronewsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = EuronewsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = EuronewsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, EuronewsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = EuronewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = EuronewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = EuronewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => EuronewsError.new(
        "#{op}_allow",
        "EuronewsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = EuronewsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = EuronewsError.new(
        "graphql_error", "EuronewsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
