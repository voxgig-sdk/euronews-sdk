...
class NewEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = EuronewsSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EURONEWS_TEST_NEW_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def new_basic_setup(extra)
  ...
  entid_env_raw = ENV["EURONEWS_TEST_NEW_ENTID"]
  ...
  if env["EURONEWS_TEST_LIVE"] == "TRUE"
    ...
    client = EuronewsSDK.new(Helpers.to_map(merged_opts))
  end
