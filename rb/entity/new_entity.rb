...
class NewEntity
  ...
  def data_set(args)
    if args
      @_data = EuronewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = EuronewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<New>, Array] the matching New items; raises EuronewsError on failure
